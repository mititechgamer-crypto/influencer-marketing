...
async function ensureSchema() {
  if (db.dialect === 'pg') {
    ...
    await db.query('ALTER TABLE user_brands ADD COLUMN IF NOT EXISTS edit_access SMALLINT NOT NULL DEFAULT 0');
    await db.query(`
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS user_brands (
        ...
        edit_access INTEGER NOT NULL DEFAULT 0,
        PRIMARY KEY (user_id, brand_id)
      )
    `);
    {
      ...
      if (!ubcols.some(c => c.name === 'edit_access')) {
        await db.query('ALTER TABLE user_brands ADD COLUMN edit_access INTEGER NOT NULL DEFAULT 0');
      }
    }
    await db.query(`
      CREATE TABLE IF NOT EXISTS influencers (
